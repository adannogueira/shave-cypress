...
let UpdateUserAvatarService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  constructor(usersRepository, storageShaver) {
    this.usersRepository = usersRepository;
    this.storageShaver = storageShaver;
  }
  ...
  async execute({
    ...
    if (user.avatar) {
      await this.storageShaver.deleteFile(user.avatar);
    }

    const fileName = await this.storageShaver.saveFile(avatarFilename);
